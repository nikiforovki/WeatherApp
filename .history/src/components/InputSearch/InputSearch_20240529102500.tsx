...
export const InputSearch = () => {
  ...
  const apiError = useSelector((state) => state.weather.apiError); // Предполагается, что ошибка хранится здесь

  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      setCityName(event.target.value.toLowerCase());
      dispatch(updateCity(cityName));
      dispatch(fetchCurrentWeatherRequest({ city: cityName })).catch(
        (error) => {
          if (error.message === 'Город не найден') {
            setErrorMessage('Город не найден');
            // Здесь вы можете также отправить ошибку в Redux store, например:
            // dispatch({ type: 'SET_API_ERROR', payload: 'Город не найден' });
          }
        },
      );
      event.target.value = '';
  ...
  useEffect(() => {
    if (apiError && apiError.includes('Город не найден')) {
      setErrorMessage('Город не найден');
    } else {
      setErrorMessage('');
    }
