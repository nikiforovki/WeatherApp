...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      // Используем useEffect для очистки поля ввода после обновления состояния selectedCity
      useEffect(() => {
        if (selectedCity !== '') {
          setSelectedCity('');
        }
  ...
  return (
    <SearchContainer>
      <StyledInput
        name='myInput'
        placeholder='Введите текст...'
        onChange={(e) => setSelectedCity(e.target.value)} // Используем onChange для управления состоянием
        onKeyPress={handleKeyPress}
        onClick={toggleCitiesList}
        value={selectedCity} // Установка значения поля ввода через состояние
      />
      <StyledIconSearch onClick={toggleCitiesList}>
        <IconSearch />
      </StyledIconSearch>
