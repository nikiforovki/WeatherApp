...
import { WORKER_GET_WEATHER_DATA_BY_SEARCH_FAILURE } from '../../Redux/actions/actions';
...
export const InputSearch = () => {
  ...
  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      const currentCity = event.target.value.trim();
  ...
  // Обработчик для действия WORKER_GET_WEATHER_DATA_BY_SEARCH_FAILURE
  const handleFailureAction = (action) => {
    if (action.type === WORKER_GET_WEATHER_DATA_BY_SEARCH_FAILURE) {
      setErrorMessage(action.error);
    }
  };

  // Подписываемся на изменения состояния глобального стора
  useEffect(() => {
    return dispatch(handleFailureAction());
  }, [dispatch]);
