...
export const InputSearch: React.FC = () => {
  ...
  // Перемещаем определение filteredCities выше
  const filteredCities = useMemo(
    () =>
      cities.filter((city) =>
        city.toLowerCase().includes(inputValue.toLowerCase()),
      ),
    [inputValue, cities], // Добавляем cities в зависимости
  );

  const [hasCities, setHasCities] = useState(filteredCities.length > 0); // Используем filteredCities здесь

  useEffect(() => {
    setHasCities(filteredCities.length > 0); // Теперь filteredCities определен
  }, [filteredCities]); // Указываем filteredCities в качестве зависимости
