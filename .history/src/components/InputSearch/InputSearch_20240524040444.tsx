...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') {
      ...
      // Проверяем, есть ли совпадения в списке городов
      const hasMatch = filtered.some(
        (city) =>
          city.toLowerCase() === event.currentTarget.value.toLowerCase(),
      );

      // Устанавливаем состояние error в зависимость от наличия совпадений
      setError(!hasMatch);
      ...
      // Очищаем поле ввода
      setSelectedCity(''); // Или использовать event.currentTarget.reset(), но это может не работать во всех браузерах
    }
  };
  ...
  return (
    <SearchContainer>
      ...
      {error && <ErrorMessage>Город не найден</ErrorMessage>}
      {showCitiesList && (
        <CitiesList>
