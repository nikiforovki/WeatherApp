...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      // Предполагается, что здесь происходит вызов функции, которая может выбросить исключение
      try {
        validateCity(currentCity); // Функция, которая может выбросить исключение, если город не найден
      } catch (error) {
        setError(true);
