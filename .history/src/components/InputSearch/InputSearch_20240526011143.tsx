...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter' && inputValue.trim()) {
      const currentCity = inputValue.trim();
      ...
      setShowCitiesList(false);
      setInputValue(''); // Сбросить значение инпута после выбора города
    }
  };
  ...
  return (
    <SearchContainer>
      <StyledInput
        ...
        onKeyPress={handleKeyPress}
        onClick={toggleCitiesList}
        value={inputValue} // Использование локального состояния для значения инпута
      />
      <StyledIconSearch onClick={toggleCitiesList}>
        <IconSearch />
      </StyledIconSearch>
        ...
        <CitiesList>
          {cities.map((city, index) => (
            <CityItem
              key={index}
              onClick={() => {
                setInputValue(city); // Обновление значения инпута при выборе города
                onCityChange(city);
                setShowCitiesList(false);
              }}
            >
