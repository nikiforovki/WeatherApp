...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const [filterText, setFilterText] = useState(''); // Шаг 1: Добавление состояния для фильтра
  const [filteredCities, setFilteredCities] = useState([
    'Москва',
    'Санкт-Петербург',
    'Новосибирск',
  ]); // Изначальный список городов

  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setFilterText(event.target.value.toLowerCase()); // Обновление фильтра при каждом изменении
    setFilteredCities(
      cities.filter((city) => city.toLowerCase().includes(filterText)),
    ); // Фильтрация городов
  };
  ...
  return (
    <SearchContainer>
      <StyledInput
        name='myInput'
        placeholder='Введите текст...'
        onChange={handleInputChange} // Изменение на onChange для отслеживания изменений
        onKeyPress={handleKeyPress}
        onClick={toggleCitiesList}
      />
      <StyledIconSearch onClick={toggleCitiesList}>
