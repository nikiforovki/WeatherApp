...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const isFirstRender = useRef(true).current; // Используйте useRef для isFirstRender, чтобы сохранить состояние между ререндерами

  const debouncedFetchCityList = useDebouncedFunction(() => {
    dispatch({
  ...
  const handleChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      debouncedFetchCityList.clear(); // Очистите предыдущие таймеры
    },
    [inputValue, debouncedFetchCityList],
  );
