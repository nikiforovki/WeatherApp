...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      const currentCity = inputValue.trim(); // Используйте inputValue вместо event.currentTarget.value

      setInputValue(currentCity); // Обновите inputValue сразу после изменения
    ...
    <ErrorBoundary>
      <SearchContainer>
        <StyledInput
          name='myInput'
          placeholder='Введите текст...'
          onChange={(e) => setInputValue(e.target.value)} // Измените обработчик onChange
          onKeyPress={handleKeyPress}
          onClick={toggleCitiesList}
          value={inputValue} // Используйте inputValue вместо selectedCity
        />
          ...
          <CitiesList>
            {filteredCities.map((city, index) => (
              <CityItem
                key={index}
                onClick={() => {
                  setInputValue(city); // Обновите inputValue при выборе города из списка
                  onCityChange(city);
                  setShowCitiesList(false);
                }}
              >
