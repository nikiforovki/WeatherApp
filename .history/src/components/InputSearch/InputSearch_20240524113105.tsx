...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      // const currentCity = event.currentTarget.value.replace(/./g, '');
  ...
  // Функция для установки ошибки поиска
  const setSearchErrorState = (errorMessage) => {
    setSearchError(errorMessage);
    // Здесь можно добавить дополнительную логику, например, сбросить другие ошибки
  };

  // Функция для установки другой ошибки
  const setOtherErrorState = (errorMessage) => {
    setOtherError(errorMessage);
    // Аналогично, здесь можно добавить дополнительную логику
  };
  ...
  return (
    <ErrorBoundary>
      <SearchContainer>
        ...
        {searchError ? (
          <ErrorMessage>Ошибка поиска: {searchError}</ErrorMessage>
        ) : null}{' '}
        {otherError ? (
          <ErrorMessage>Другая ошибка: {otherError}</ErrorMessage>
        ) : null}{' '}
        {showCitiesList && (
          <CitiesList>
