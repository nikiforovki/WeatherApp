...
export const InputSearch: React.FC = () => {
  ...
  const city = useSelector((state) => state.city); // Получаем city из состояния Redux

  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') {
      const currentCity = event.currentTarget.value.toLowerCase();
      dispatch(updateCity(currentCity)); // Отправляем экшен updateCity
    }
  };
    ...
    <ErrorBoundary>
      <SearchContainer>
        <StyledInput
          ...
          defaultValue={city} // Используем city в качестве начального значения
        />
        <StyledIconSearch>
          <IconSearch />
        </StyledIconSearch>
