...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      // Logic executed before clearing the input
      setSelectedCity(currentCity); // Update state immediately
      ...
      // Clearing the input field after selecting a city and pressing Enter
      // Note: The line below is commented out to avoid unnecessary re-renders
      // setSelectedCity(''); // Uncomment this line if you want to clear the input field
    }
  };
  ...
  return (
    <SearchContainer>
      <StyledInput
        ...
        onKeyPress={handleKeyPress}
        onClick={toggleCitiesList}
        value={selectedCity} // Добавьте этот проп для связи состояния с полем ввода
      />
      <StyledIconSearch onClick={toggleCitiesList}>
        <IconSearch />
      </StyledIconSearch>
