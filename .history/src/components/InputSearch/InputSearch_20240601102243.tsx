...
export const InputSearch = () => {
  ...
  const [cityNotFound, setCityNotFound] = useState(false); // Добавляем состояние для отслеживания ошибки
  ...
  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      ...
      // Используем блок try/catch для перехвата ошибок
      try {
        // Искусственное создание ошибки для тестирования
        throw new Error('город не найден');
      } catch (error) {
        setCityNotFound(true); // Устанавливаем флаг ошибки
      }
  ...
  return (
    <ErrorBoundary>
      ...
      {/* Отображаем запасной UI, если ошибка произошла */}
      {cityNotFound && (
