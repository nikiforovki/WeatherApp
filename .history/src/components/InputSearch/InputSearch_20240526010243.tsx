...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      setSelectedCity(currentCity); // Используем только selectedCity для управления инпутом
      ...
      setShowCitiesList(false);
      setSelectedCity(''); // Сбросим значение после использования, если необходимо
    }
  };
    ...
    <ErrorBoundary>
      <SearchContainer>
        <StyledInput
          name='myInput'
          placeholder='Введите текст...'
          onChange={(e) => setSelectedCity(e.target.value)} // Используем selectedCity для управления инпутом
          onKeyPress={handleKeyPress}
          onClick={toggleCitiesList}
          value={selectedCity} // Значение инпута управляется через selectedCity
        />
        <StyledIconSearch onClick={toggleCitiesList}>
          <IconSearch />
        </StyledIconSearch>
        ...
        {showCitiesList && (
          <CitiesList>
            {cities.map(
              (
                city,
                index, // Используем cities напрямую для отображения списка
              ) => (
                <CityItem
                  key={index}
                  onClick={() => {
                    setSelectedCity(city); // Обновляем selectedCity при выборе города
                    onCityChange(city);
                    setShowCitiesList(false);
                  }}
                >
...
//           <IconSearch />
//         </StyledIconSearch>
//         {error ? <ErrorMessage>Город не найден</ErrorMessage> : null}{' '}
//         {showCitiesList && (
//           <CitiesList>
