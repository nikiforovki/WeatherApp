  ...
  const fetchCities = async () => {
    try {
      const response = await axios.get(
        `http://api.openweathermap.org/geo/1.0/direct?q=Moscow,RU&limit=5&lang=ru&appid=${process.env.REACT_APP_API_KEY}`,
      );
    ...
    <SearchContainer>
      ...
      {showCitiesList && (
        <CitiesList>
          {cities.map((city) => (
            <CityItem
              key={city} // Используем уникальный идентификатор в качестве ключа
              onClick={() => {
