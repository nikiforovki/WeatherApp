...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter') {
      setSelectedCity(event.currentTarget.value.toLowerCase()); // Сохраняем текущее значение в lowercase
      ...
      onCityChange(selectedCity); // Передаем текущее значение города в callback
      setShowCitiesList(false);
