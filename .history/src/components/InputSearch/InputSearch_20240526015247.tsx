...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const cities = [
    { name: 'Москва', timezone: 'Europe/Moscow' }, // Пример данных с таймзонами
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      const hasMatch = filtered.some(
        (city) => city.name.toLowerCase() === currentCity,
      );
      setError(!hasMatch);
      onCityChange({ city: currentCity, timezone: filtered[0]?.timezone }); // Передаем объект с городом и таймзоной
      setShowCitiesList(false);
      setSelectedCity('');
    }
  };
          ...
          <CitiesList>
            {filteredCities.map((city, index) => (
              <CityItem
                ...
                onClick={() => {
                  setSelectedCity(city.name);
                  onCityChange({ city: city.name, timezone: city.timezone }); // Передаем объект с городом и таймзоной
                  setShowCitiesList(false);
                }}
              >
                {city.name}
