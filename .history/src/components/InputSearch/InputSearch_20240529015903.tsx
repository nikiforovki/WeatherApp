...
export const InputSearch = () => {
  ...
  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      ...
      dispatch(updateCity(currentCity)); // Обновление города в состоянии
      dispatch(fetchCurrentWeatherRequest({ city: currentCity })); // Запрос текущей погоды

      // Инициация запроса погоды по названию города
      dispatch(
        fetchWeatherDataByCityNameRequest({
          cityName: currentCity,
          weatherUnit: 'metric', // Указание единиц измерения
        }),
      );
