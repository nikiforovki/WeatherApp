...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const debouncedFetchCityList = useCallback(
    _.debounce((value) => {
      dispatch({
  ...
  const handleChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      debouncedFetchCityList(event.target.value); // Используйте debounced версию здесь
    },
    [setInputValue, setShowDropdown, debouncedFetchCityList],
  );
  const debouncedHandleChange = useCallback(debounce(handleChange, 300), [
    handleChange,
    dispatch,
  ]); // Добавление декоратора debounce
    ...
    <>
      <SearchContainer>
        <StyledInput
          ...
          onChange={debouncedHandleChange} // Использование debounced version
          onFocus={handleFocus}
          onBlur={handleBlur}
        />
        <StyledIconSearch>
