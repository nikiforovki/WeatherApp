...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  const [showCitiesList, setShowCitiesList] = useState(false);
  const [selectedCity, setSelectedCity] = useState(''); // Инициализация с пустой строкой
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === 'Enter') {
      ...
      setSelectedCity(currentCity); // Обновление состояния с текущим значением инпута
      ...
      setShowCitiesList(false);
      setSelectedCity(''); // Сброс значения после выбора города
    }
  };
  ...
  return (
    <SearchContainer>
      <StyledInput
        name='myInput'
        placeholder='Введите текст...'
        onChange={(e) => setSelectedCity(e.target.value)} // Связывание с состоянием
        onKeyPress={handleKeyPress}
        onClick={toggleCitiesList}
        value={selectedCity} // Использование состояния для значения инпута
      />
      <StyledIconSearch onClick={toggleCitiesList}>
        <IconSearch />
      </StyledIconSearch>
        ...
        <CitiesList>
          {filteredCities.map((city, index) => (
            <CityItem
              key={index}
              onClick={() => {
                setSelectedCity(city); // Обновление значения инпута при выборе города
                onCityChange(city);
                setShowCitiesList(false);
              }}
            >
