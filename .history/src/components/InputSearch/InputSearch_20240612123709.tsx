...
import {
  selectErrorMessage,
  selectCityNames,
} from '../../Redux/slice/currentweatherSlice'; // Предполагается, что эти селекторы определены где-то в вашем проекте
import {
  updateCity,
  fetchCurrentWeatherRequest,
  WORKER_GET_WEATHER_DATA_BY_SEARCH_REQUEST,
} from '../../Redux/actions/actions'; // Предполагается, что эти действия определены где-то в вашем проекте
...
export const InputSearch: React.FC = () => {
  ...
  const handleCityChange = useCallback(
    (city: string) => {
      console.log(`Searching for city: ${city}`); // Логирование для диагностики
      setInputValue(city);
  ...
  useEffect(() => {
    // Автоматический поиск города при монтировании компонента
    const defaultCity = 'Berlin';
    console.log(`Setting default city to ${defaultCity}`); // Логирование для диагностики
    handleCityChange(defaultCity);
  }, []); // Этот эффект должен вызываться только при монтировании
