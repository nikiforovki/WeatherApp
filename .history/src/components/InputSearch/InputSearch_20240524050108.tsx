...
export const InputSearch: React.FC<InputSearchProps> = ({ onCityChange }) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    console.log('Key pressed:', event.key); // Отладочный лог
    if (event.key === 'Enter') {
      const currentCity = event.currentTarget.value.toLowerCase();

      // Здесь выполняется вся логика, которую нужно выполнить до очистки инпута
      setSelectedCity(currentCity); // Обновляем состояние немедленно
      ...
      // Очищаем поле ввода после выбора города и нажатия Enter
      setSelectedCity(''); // Раскомментируйте эту строку, если хотите очистить поле ввода
  ...
  return (
    <SearchContainer>
      <StyledInput
        ...
        onKeyPress={handleKeyPress}
        onClick={toggleCitiesList}
        value={selectedCity} // Убедитесь, что StyledInput использует value для отображения текущего значения
      />
      <StyledIconSearch onClick={toggleCitiesList}>
        <IconSearch />
      </StyledIconSearch>
