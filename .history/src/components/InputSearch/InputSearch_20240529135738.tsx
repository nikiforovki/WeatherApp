...
const InputSearch = () => {
  ...
  const error = useSelector((state) => state.weather.error); // Используйте селектор для получения ошибки
  ...
  useEffect(() => {
    if (error && error.includes('Город не найден')) {
      alert(error); // Можно заменить на более дружественный интерфейс
    }
  }, [error]);
