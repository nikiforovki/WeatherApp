...
export const InputSearch: React.FC = () => {
  ...
  const [inputValue, setInputValue] = useState('Москва'); // Начальное значение "Москва"
  ...
  useEffect(() => {
    // Запрос погоды на Москву при загрузке страницы
    dispatch(fetchCurrentWeatherRequest({ city: 'Москва' }));
  }, []); // Пустой массив зависимостей означает, что эффект запускается один раз при монтировании

  const handleCityChange = useCallback(
    (city: string) => {
      setInputValue(city);
      dispatch(updateCity(city)); // Обновление состояния города в Redux
