...
import ErrorBoundary from '../ErrorBoundary/ErrorBoundary';
import IconSearch from '../../../public/assets/images/IconSearch'; // Убедитесь, что этот путь корректен
...
export const InputSearch = () => {
  ...
  const error = useSelector((state) => state.weather?.error); // Использование опциональной цепочки
  ...
  const handleKeyPress = (event) => {
    if (event.key === 'Enter') {
      ...
      event.target.value = ''; // Очищаем поле ввода после отправки запроса
    }
  };
