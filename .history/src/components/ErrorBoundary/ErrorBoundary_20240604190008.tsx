...
class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  // Этот метод вызывается при возникновении ошибки в дочерних компонентах
  static getDerivedStateFromError(error: Error): ErrorBoundaryState {
    // Проверяем, содержит ли сообщение об ошибке строку 'город не найден'
    if (error.message.includes('город не найден')) {
      // Если да, устанавливаем hasError в false, предполагая, что это специфический случай, который мы хотим игнорировать
      return { hasError: false };
    }
    // Во всех остальных случаях устанавливаем hasError в true
    return { hasError: true };
  }

  // Этот метод вызывается после того, как ошибка была перехвачена
