...
export const TemperatureScaleToggle = () => {
  ...
  const temperature = weatherData?.current?.temp ?? 0;
  // const temperature = useSelector((state) => state.weatherData.current.temp);
  const units = useSelector((state) => state.weatherData.units);
  ...
  return (
    <ToggleContainer onClick={handleClick}>
      ...
      <div>
        <h1>
          Текущая температура: {temperature} {units}
        </h1>
