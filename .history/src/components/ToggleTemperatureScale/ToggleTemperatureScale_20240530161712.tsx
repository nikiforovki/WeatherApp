...
export const TemperatureScaleToggle = ({ disabled = false }) => {
  ...
  const temperatureScale = useSelector((state) => state.temperatureScale); // Изменение масштаба температуры в состоянии Redux

  const handleClick = () => {
    if (!disabled) {
      const newScale = temperatureScale === 'C' ? 'F' : 'C';
      dispatch(toggleTemperatureScale(newScale === 'C' ? 0 : 1)); // Отправка действия для изменения масштаба
    }
  };

  return (
    <button onClick={handleClick} disabled={disabled}>
      {temperatureScale === 'C' ? 'C' : 'F'}
    </button>
  );
};
