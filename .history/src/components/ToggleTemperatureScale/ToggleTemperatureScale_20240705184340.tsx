...
export const TemperatureScaleToggle = () => {
  ...
  useEffect(() => {
    console.log('Current weather state:', weatherState);
  }, [weatherState]); // Зависимость от weatherState, чтобы логировать каждый раз, когда состояние меняется

  if (!weatherUnit || isLoading || hasError) {
    return <p>Loading...</p>;
  }
