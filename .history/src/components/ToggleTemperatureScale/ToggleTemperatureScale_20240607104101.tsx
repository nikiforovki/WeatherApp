...
export const TemperatureScaleToggle = () => {
  ...
  const handleClick = () => {
    ...
    dispatch(toggleTemperatureScale(newScale === 'C')); // Исправлено здесь
  };

  return (
    <ToggleContainer onClick={handleClick}>
      <ActiveToggleButton>{isCelsius ? 'C' : 'F'}</ActiveToggleButton>
      <SlashButton>/</SlashButton>
      <InactiveToggleButton>{isCelsius ? 'F' : 'C'}</InactiveToggleButton>
    </ToggleContainer>
  );
};
