...
export const TemperatureScaleToggle = () => {
  ...
  const handleClick = () => {
    const newScale = temperatureScale === 'C' ? 'F' : '';
    dispatch(toggleTemperatureScale(newScale === '' ? 0 : 1));
  };

  return (
    <ToggleContainer onClick={handleClick}>
      <ActiveToggleButton>{temperatureScale}</ActiveToggleButton>
      {/* Изменено: теперь SlashButton всегда отображает слэш */}
      <SlashButton>/</SlashButton>
      {/* Изменено: теперь InactiveToggleButton отображает 'F' или 'C' в зависимости от состояния */}
