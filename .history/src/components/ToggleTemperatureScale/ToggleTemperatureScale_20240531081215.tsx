...
export const TemperatureScaleToggle = () => {
  ...
  const temperatureScale = useSelector(
    (state) => state.weatherData?.temperatureScale || 'C',
  ); // Предполагается, что состояние находится в weatherData
  const [isActive, setIsActive] = useState(false); // Определение isActive как части состояния

  const handleClick = () => {
    const newScale = temperatureScale === 'C' ? 'F' : 'C';
    dispatch(toggleTemperatureScale(newScale));
    setIsActive(!isActive); // Переключение isActive при каждом клике
  };

  return (
    <button onClick={handleClick}>
      {temperatureScale}°{temperatureScale === 'C' ? 'C' : 'F'}
    </button>
  );
};
// import React, { useState } from 'react';
