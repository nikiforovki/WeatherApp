...
import { putWeatherData, setTemperatureScale } from '../../Redux/actions/actions';
...
const TemperatureScaleToggle = () => {
  ...
  const temperatureScale = useSelector((state: State) => state.temperatureScale);
  ...
  const handleClick = () => {
    ...
    // Обновляем состояние шкалы измерения
    dispatch(setTemperatureScale(newScale));

    // Обновляем данные о погоде с новой шкалой измерения
    dispatch(putWeatherData({
      currentTemp: /* Получите текущую температуру */,
      windSpeed: /* Получите скорость ветра */,
      unit: newScale,
    }));
  };
