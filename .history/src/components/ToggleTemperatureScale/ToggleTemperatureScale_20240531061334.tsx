...
import { selectTemperature } from '../../Redux//slice/currentweatherSlice'; // Путь к вашему файлу селекторов
...
const temperatureScale = useSelector(selectTemperature); // Получаем текущую шкалу из Redux-хранилища
  ...
  const handleClick = () => {
    dispatch(toggleTemperature()); // Отправляем действие для переключения шкалы
  };
  ...
  return (
    <ToggleContainer onClick={handleClick}>
      ...
      <InactiveToggleButton>{temperatureScale === 'C'? 'F' : 'C'}</InactiveToggleButton>
    </ToggleContainer>
  );
//   return (
//     <ToggleContainer onClick={handleClick}>
