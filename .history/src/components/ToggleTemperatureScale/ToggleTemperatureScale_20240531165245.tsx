...
export const TemperatureScaleToggle = () => {
  ...
  // Явно указываем тип состояния, используя RootState
  const temperatureScale = useSelector<RootState>(
    (state) => state.weatherReducer?.temperatureScale,
  );
  console.log('Температура1111', temperatureScale);

  if (!temperatureScale) {
    return <p>Загрузка...</p>; // Запасной UI вместо null
  }

  const handleClick = () => {
    // Переключаем температуру на противоположную
