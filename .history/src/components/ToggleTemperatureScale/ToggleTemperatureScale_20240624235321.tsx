...
// Определяем стили для кнопки внутри компонента
const TemperatureToggleButton = styled.button`
  ...
  background: none;
  color: ${({ isCelsius }) => (isCelsius ? '#1AAFE0' : '#000')};
`;

export const TemperatureScaleToggle = () => {
  const dispatch = useDispatch();
  // Явно указываем тип состояния для useSelector
  const temperatureScale = useSelector(
    (state: { temperatureScale: string }) => state.temperatureScale,
  );
  ...
  const handleClick = () => {
    ...
    // Убедимся, что передаем правильный тип данных
