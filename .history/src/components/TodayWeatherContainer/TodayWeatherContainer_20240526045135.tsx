...
const TodayWeatherContainer: React.FC = () => {
  ...
  const [temperatureScale, setTemperatureScale] = useState<'C' | 'K'>('C'); // Инициализируем состояние масштаба температуры
  ...
  // Функция для отображения температуры в выбранном масштабе
  const displayTemperature = () => {
    if (temperatureScale === 'C') {
      return Math.floor(TemperatureToday - 273.15); // Перевод из Кельвинов в Цельсии
    } else {
      return TemperatureToday; // Возвращаем температуру в Кельвинах
    }
  };
