...
const TodayWeatherContainer: React.FC = () => {
  ...
  const [temperatureScale, setTemperatureScale] = useState<'C' | 'F'>('C'); // Инициализация состояния шкалы
  ...
  cconst displayTemperature = () => {
    ...
    let baseTemperature = TemperatureToday; // Используем прямое значение из KELVIN без конвертации
    if (temperatureScale === 'F') {
      baseTemperature = convertCelsiusToFahrenheit(convertKelvinToCelsius(TemperatureToday)); // Конвертируем только если выбрана шкала F
    }
    console.log('Converted Temperature:', baseTemperature); // Логирование результата конвертации
