...
const TodayWeatherContainer: React.FC = () => {
  ...
  const [hasConvertedOnce, setHasConvertedOnce] = useState(false); // Добавлено новое состояние
  ...
  const convertKelvinToCelsius = (kelvin: number) => {
    if (!hasConvertedOnce) {
      return Math.round(kelvin - 273.15); // Выполняем конвертацию только при первом запуске
      setHasConvertedOnce(true); // Обновляем состояние, чтобы конвертация не повторялась
    }
    return kelvin; // Возвращаем исходное значение, если конвертация уже была выполнена
  };

  const displayTemperature = () => {
    let celsius = convertKelvinToCelsius(TemperatureToday);
