...
const TodayWeatherContainer: React.FC = () => {
  ...
  const [TemperatureToday, setTemperatureToday] = useState<number | null>(null); // Используем состояние для хранения температуры
  console.log('232343Temperature', TemperatureToday);
  ...
  useEffect(() => {
    if (weatherData && weatherData.current) {
      ...
      setTemperatureToday(temp); // Устанавливаем температуру здесь
    }
  }, [weatherData]); // Зависимость от weatherData
