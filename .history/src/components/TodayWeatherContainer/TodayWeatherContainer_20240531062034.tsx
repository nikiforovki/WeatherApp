...
import { selectWeatherData } from '../../Redux/slice/currentweatherSlice';
...
const TodayWeatherContainer: React.FC = () => {
  ...
  const currentCity = 'Berlin'; // Пример значения города, можно сделать динамическим
  const temperatureScale = useSelector(selectTemperature); // Получаем текущую шкалу из Redux-хранилища
  ...
  const displayTemperature = () => {
    if (temperatureScale === 'C') {
      return Math.floor(weatherData.current.temp);
    } else if (temperatureScale === 'F') {
      return convertCelsiusToFahrenheit(weatherData.current.temp);
    } else {
      return Math.floor(weatherData.current.temp);
    }
  };
  ...
  return (
    <Container>
      ...
      <TemperatureScaleToggle /> {/* Теперь компонент использует Redux */}
