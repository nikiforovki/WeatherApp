...
const TodayWeatherContainer: React.FC = () => {
  ...
  if (!weatherData && !showError) {
    return (
      <Container>
        <Skeleton /> {/* Скелетон для заголовка */}
        <Skeleton style={{ height: '108px', width: '50%' }} />{' '}
        {/* Скелетон для температуры */}
        <Skeleton style={{ height: '31px', width: '50%' }} />{' '}
        {/* Скелетон для города */}
        {/* Добавьте дополнительные скелетоны по необходимости */}
      </Container>
    );
  }
