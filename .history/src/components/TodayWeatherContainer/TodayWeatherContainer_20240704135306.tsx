...
const TodayWeatherContainer: React.FC = () => {
  ...
  const TemperatureToday = useSelector(selectTemperature); // Измените селектор, если он зависит от шкалы
  ...
  const displayTemperature = () => {
    if (temperatureScale === 'metric') {
      ...
      return Math.round(convertCelsiusToFahrenheit(TemperatureToday)); // Предполагается, что у вас есть функция convertCelsiusToFahrenheit
    } else {
      return Math.round(TemperatureToday);
    }
  };
