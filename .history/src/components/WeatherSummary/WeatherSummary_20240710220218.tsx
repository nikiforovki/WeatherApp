...
export const WeatherSummary = () => {
  ...
  useEffect(() => {
    try {
      fetchWeatherData('New York'); // Пример города для запроса
    } catch (err) {
      setError(true); // Устанавливаем состояние ошибки в true, если произошла ошибка
    }
  }, []);

  if (error) {
    return null; // Не рендерим компонент, если произошла ошибка
  }
