...
const HourlyWeatherForecast = () => {
  ...
  useEffect(() => {
    if (hourlyTemp && hourlyTemp.length >= 7) {
      ...
      setForecastData(
        firstSevenTemps.map((temp, index) => ({
          time: `${index + 12} pm`,
  ...
  const firstSevenTempsCelsius = firstSevenTemps.map(
    ...
    console.log('температур:', firstSevenTemps),
  );
  ...
  const getWeatherIcon = (hourlyData) => {
    // Предполагаем, что hourlyData содержит поле description
    const description = hourlyData.description;
      ...
      {forecastData.map((item, index) => (
        <WeatherCard key={index}>
          <StyledBoxText>
            ...
            <StyledTemperature>
              {item.temperature}°{temperatureScale}
            </StyledTemperature>
