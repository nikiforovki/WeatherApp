  ...
  return (
    <Container>
      {forecastData.map((item) => (
        <WeatherCard key={item.id}> {/* Используйте уникальный ключ */}
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}> {/* Строка для элементов */}
            <StyledTime>{item.time}</StyledTime>
            <StyledTemperature>{item.temperature}</StyledTemperature>
            <StyledIconСloudСomputing>
              <img src="/IconСloudСomputing.svg" alt="Cloud Computing Icon" /> {/* Используйте img для SVG */}
            </StyledIconСloudСomputing>
          </div>
        </WeatherCard>
      ))}
