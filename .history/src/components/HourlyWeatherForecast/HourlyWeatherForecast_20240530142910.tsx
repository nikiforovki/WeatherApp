...
const HourlyWeatherForecast = () => {
  ...
  const temperatureScale = useSelector(selectTemperatureScale); // Получаем текущую шкалу температуры
  // const selectTemperatureScale = (state) => state.weather.temperatureScale;
  // console.log('selectTemperatureScale:', selectTemperatureScale);
  ...
  const firstSevenTempsInSelectedScale = firstSevenTemps.map((temp) => {
    if (temperatureScale === 'C') {
      ...
      // Преобразование в Фаренгейты, если выбрана шкала F
      return Math.floor((temp * 9) / 5 + 32);
    }
  });

  const firstSevenTempsCelsius = firstSevenTemps
    .map(Math.floor)
