...
const HourlyWeatherForecast = () => {
  ...
  const getWeatherIcon = (description: string) => {
    // Логика выбора иконки погоды
  };

  return (
    <Container>
      <TemperatureScaleToggle onScaleChange={setTemperatureScale} /> {/* Кнопка переключения шкалы температуры */}
      {forecastData.map((item, index) => (
        <WeatherCard key={index}>
          <StyledBoxText>
            <StyledTime>{item.time}</StyledTime>
            <StyledTemperature>{`${item.temperature}°${temperatureScale}`}</StyledTemperature>
            <StyledIconСloudСomputing>
              {hourlyData && getWeatherIcon(hourlyData)}
            </StyledIconСloudСomпутиng>
          </StyledBoxText>
        </WeatherCard>
      ))}
    </Container>
