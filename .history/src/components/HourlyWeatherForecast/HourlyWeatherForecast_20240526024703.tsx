...
const HourlyWeatherForecast = () => {
  ...
  console.log('Первые семь температур:', firstSevenTemps);

  // Извлечение первого элемента из firstSevenTemps
  const firstTemp = firstSevenTemps[0];

  // Обновление forecastData, заменяя температуру первого объекта на значение из firstTemp
  const updatedForecastData = forecastData.map((item, index) => {
    // Если это первый объект, заменяем его температуру
    if (index === 0) {
      return { ...item, temperature: firstTemp };
    }
    // Для остальных объектов возвращаем их без изменений
    return item;
  });

  // Обновляем состояние forecastData новым массивом
