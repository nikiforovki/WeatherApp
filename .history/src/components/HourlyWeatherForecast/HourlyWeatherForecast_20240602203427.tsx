...
const HourlyWeatherForecast = () => {
  ...
  console.log('Код иконки по часам:', descriptionIconsHuorly);
  ...
  useEffect(() => {
    if (hourlyTemp && hourlyTemp.length >= 7) {
      ...
      // Создаем массив временных меток, добавляя к текущему времени индекс элемента
      const times = temps.map((_, index) => {
        const adjustedTime = new Date(currentTime.getTime()); // Копируем текущее время
        adjustedTime.setSeconds(adjustedTime.getSeconds() + index); // Добавляем секунды, равные индексу
        return formatDate(adjustedTime); // Форматируем дату и время
      });

      setForecastData(
        temps.map((temp, index) => ({
          time: times[index], // Используем сформатированное время
          temperature: Math.round(temp),
          iconCode: icons[index],
        })),
      );
