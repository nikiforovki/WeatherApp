...
const HourlyWeatherForecast = () => {
  ...
  const [isDataLoaded, setIsDataLoaded] = useState(false); // Инициализируем как false
  ...
  useEffect(() => {
    setIsDataLoaded(false); // Устанавливаем в false перед загрузкой данных
    if (hourlyTemp && hourlyTemp.length >= 7) {
      ...
      setIsDataLoaded(true); // Устанавливаем в true после загрузки данных
    }
