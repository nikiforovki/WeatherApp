  ...
  useEffect(() => {
    if (hourlyData) {
      const formattedData = hourlyData.map((item) => ({
        time: new Date(item.dt * 1000).toLocaleTimeString(), // Преобразование Unix timestamp в читаемый формат времени
        temperature: item.main.temp,
        description: item.weather[0].description, // Извлечение описания погоды
      }));
        ...
        <WeatherCard key={index}>
          <StyledBoxText>
            ...
            <StyledIconСloudСomputing>
              {item.description && getWeatherIcon(item.description)}
            </StyledIconСloudСomputing>
          </StyledBoxText>
        </WeatherCard>
      ))}
