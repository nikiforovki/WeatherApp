...
import { useSelector } from 'react-redux'; // Предполагается, что у вас есть другой селектор для получения данных
...
const HourlyWeatherForecast: React.FC = () => {
  // Предполагается, что у вас есть другой селектор для получения данных
  const weatherData = useSelector(yourOtherSelectorHere);
  ...
  function convertToFahrenheitToCelsius(tempInFahrenheit) {
    return ((tempInFahrenheit - 32) * 5) / 9;
  }

  const getWeatherIcon = (description) => {
    if (!description) return <WeatherIcon />;
    switch (description) {
      case 'clear sky':
        return <SunIcon />;
      case 'few clouds':
  ...
  return (
    <Container>
      {weatherData.firstSixHours.map((hour: HourlyWeather, index: number) => (
        <WeatherCard key={index}>
          <StyledTemp>{hour.temp.toFixed(2)}°C</StyledTemp>
          <StyledTime style={{ marginLeft: '10px' }}>
            ({convertMillisecondsToTime(hour.dt)})
          </StyledTime>
