...
const HourlyWeatherForecast = () => {
  ...
  console.log('Массив температуры', hourlyTemp);
  ...
  useEffect(() => {
    ...
    if (hourlyTemp && hourlyTemp.length >= 7) {
      // Упрощено условие, так как уже проверено, что hourlyTemp имеет длину 7 или более
      const temps = hourlyTemp.slice(0, 7);
    ...
    <Container>
      ...
      {loading ? (
        <SkeletonTheme color='#FFFFFF' highlightColor='#1AADE3'>
          <SkeletonWrapper>
            {/* Количество скелетов соответствует количеству прогнозов */}
            {[...Array(7)].map((_, index) => (
...
export default React.memo(HourlyWeatherForecast);
