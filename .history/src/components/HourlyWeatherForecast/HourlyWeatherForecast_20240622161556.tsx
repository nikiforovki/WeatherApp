...
const HourlyWeatherForecast = () => {
  ...
  const forecastData = useSelector(selectForecastData); // Используйте селектор для получения forecastData, если он существует
  const descriptionIconsHuorly = useSelector(selectDescriptionIconHurly);
  const showError = useSelector((state) => state.showError); // Используйте селектор для получения флага ошибки, если он существует
  ...
  if (showError) {
    return <div>Error occurred</div>; // Показать сообщение об ошибке
  }
    ...
    <Container>
      ...
      {forecastData &&
        ...
        forecastData.map((item, index) => (
          <WeatherCard key={index} {...item} /> // Передача props напрямую
        ))}
    </Container>
  );
};
