...
const HourlyWeatherForecast = () => {
  ...
  const descriptionIcons = useSelector(selectDescriptionIcons); // Используем новый селектор для иконок
      ...
      const temps =
        temperatureScale === 'C'
         ? hourlyTemp.slice(0, 7)
          : hourlyTemp.slice(0, 7).map((temp) =>
              Math.floor((temp * 9) / 5 + 32),
            );
      const icons = descriptionIcons.slice(0, 7); // Используем массив иконок
      setForecastData(
  ...
  }, [hourlyTemp, temperatureScale, descriptionIcons]);
