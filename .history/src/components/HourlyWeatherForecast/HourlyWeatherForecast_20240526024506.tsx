...
const HourlyWeatherForecast = () => {
  ...
  // Предполагается, что firstSevenTemps уже содержит данные в нужном формате
  const updatedForecastData = forecastData.map((item, index) => {
    // Создаем новый объект с тем же временем, но с обновленной температурой из firstSevenTemps
    return { ...item, temperature: firstSevenTemps[index] || item.temperature };
  });
  ...
  const [forecastData, setForecastData, setForecastData] = useState([
