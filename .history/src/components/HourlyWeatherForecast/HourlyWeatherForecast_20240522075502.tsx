...
import { selectCurrentWeatherDetails } from '../../Redux/slice/currentweatherSlice';
...
const HourlyWeatherForecast: React.FC = () => {
  ...
  useEffect(() => {
    if (weatherData && weatherData.firstSixHours) {
      const formattedData = weatherData.firstSixHours.map((hour) => ({
        time: new Date(hour.dt * 1000).toLocaleTimeString(), // Преобразование Unix timestamp в читаемый формат времени
        temperature: hour.temp,
        icon: getWeatherIcon(hour.weather[0].description), // Использование первого элемента массива weather для получения описания
      }));
    ...
    <Container>
      {forecastData.map((item, index) => (
        <WeatherCard key={index}>
          <StyledTemp>{item.temperature}°C</StyledTemp>
