...
const SpecialWeatherCard = styled(WeatherCard)`
  background-color: #add8e6; /* Пример синего цвета */
  &:hover {
    cursor: pointer;
    opacity: 0.8;
  }
...
const SpecialWeatherCard = styled(WeatherCard)`
  background-color: #add8e6; // Изменено на правильный синий цвет
  &:hover {
    cursor: pointer;
    opacity: 0.8;
  }
...
const HourlyWeatherForecast = () => {
  ...
  return (
    <Container>
      <StyledLine />
      <TemperatureScaleToggle onScaleChange={(scale) => dispatch({ type: 'SET_TEMPERATURE_SCALE', payload: scale })} />
      {loading? (
