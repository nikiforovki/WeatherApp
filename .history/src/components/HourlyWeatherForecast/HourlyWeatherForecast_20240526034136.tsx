...
const HourlyWeatherForecast = () => {
  ...
  // Используем useEffect для обработки данных при их изменении
  useEffect(() => {
    if (hourlyTemp.length > 0) {
      const forecastData = hourlyTemp.slice(0, 7).map((temp, index) => ({
        time: `${index + 12} pm`,
        temperature: temp,
      }));
        ...
        <WeatherCard key={index}>
          <StyledBoxText>
            ...
            <StyledIconСloudСомпутинг>
              {hourlyData && getWeatherIcon(hourlyData[index].description)}{' '}
              {/* Предполагаем, что hourlyData содержит объекты с полем description */}
            </StyledIconСloudСомпутинг>
          </StyledBoxText>
        </WeatherCard>
      ))}
