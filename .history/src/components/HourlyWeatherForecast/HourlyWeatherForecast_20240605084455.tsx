...
const HourlyWeatherForecast = () => {
  ...
  useEffect(() => {
    console.log('Current error state type:', typeof error, 'value:', error);
    if (
      error ||
      !hourlyTemp ||
      hourlyTemp.length === 0 ||
      !descriptionIconsHuorly ||
      descriptionIconsHuorly.length === 0
    ) {
      setShowError(true);
    } else {
      setShowError(false);
    }
  }, [error, hourlyTemp, descriptionIconsHuorly]);
  ...
  if (showError) {
    return <div>111Произошла ошибка. Пожалуйста, попробуйте позже.</div>;
  }
