...
const HourlyWeatherForecast = () => {
  ...
  const hourlyTemp = useSelector((state) => state.hourlyTemp); // Adjust according to your state structure
  const temperatureScale = useSelector((state) => state.temperatureScale);
  const [forecastData, setForecastData] = useState([]);
  const descriptionIconsHuorly = useSelector(
    (state) => state.descriptionIconsHuorly,
  ); // Adjust according to your state structure
  const [showError, setShowError] = useState(false);
  const error = useSelector((state) => state.error); // Adjust according to your state structure
  const hourlyData = useSelector((state) => state.hourlyData); // Adjust according to your state structure
  ...
  useEffect(() => {
    ...
    if (hourlyTemp && hourlyTemp.length >= 7) {
      ...
      const formattedData = temps.map((temp, index) => ({
        time: unixTimestampToTime(hourlyData[index]),
  ...
  }, [hourlyTemp, temperatureScale, descriptionIconsHuorly, hourlyData]);
  ...
  if (showError) {
    return <div>Error loading data</div>; // Display an error message or component
  }
