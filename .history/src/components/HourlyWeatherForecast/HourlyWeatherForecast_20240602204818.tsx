...
const HourlyWeatherForecast = () => {
  ...
  // Функция для преобразования эпохи Unix в часы, минуты и секунды
  const unixTimestampToTime = (timestamp) => {
    const date = new Date(timestamp * 1000); // Умножаем на 1000, чтобы получить миллисекунды
    const hours = date.getHours();

    return `${hours}`; // Форматируем время в читаемый формат
  };
  console.log(unixTimestampToTime(hourlyData[0].time));

  // Применяем функцию к каждому элементу в hourlyData
  const formattedTimes = hourlyData.map((data) => ({
    ...data,
    time: unixTimestampToTime(data.time), // Предполагается, что у каждого элемента есть поле 'time' с эпохой Unix
  }));

  // Теперь formattedTimes содержит объекты с преобразованными временами
