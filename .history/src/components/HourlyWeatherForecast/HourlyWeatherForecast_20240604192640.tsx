...
const HourlyWeatherForecast = () => {
  ...
  const forecastData = useSelector(selectForecastData); // Предполагается, что selectForecastData - это селектор для получения прогноза погоды
  ...
  // Условный рендеринг: если forecastData пустое, отображаем заглушку или ничего
  if (forecastData.length === 0) {
    return <div>Загрузка данных...</div>; // Можно заменить на любой другой компонент или элемент
  }
...
//     // Получаем время в 12-часовом формате с AM/PM
//     const options = { hour: 'numeric', hour12: true };
//     return date.toLocaleTimeString(undefined, options);
//   }
...
//   // Преобразование каждого Unix timestamp в нормальное время
//   const formattedTimes = hourlyData.map(unixTimestampToTime);

//   // console.log('Отформатированное время:', formattedTimes);
