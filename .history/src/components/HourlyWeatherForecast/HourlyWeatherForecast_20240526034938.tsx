...
const HourlyWeatherForecast = () => {
  const hourlyData = useSelector(selectHourlyData); // Получаем данные о погоде из Redux store
  const hourlyTemp = useSelector(selectHourlyTemp); // Получаем данные о температуре из Redux store
  ...
  useEffect(() => {
    if (
      ...
      const forecastData = hourlyData.slice(0, 7).map((data, index) => ({
        time: `${index + 12} pm`, // Генерируем время, например, "12 pm", "1 pm" и т.д.
        temperature: hourlyTemp[index], // Используем температуру из массива
        icon: getWeatherIcon(data.description), // Определяем иконку погоды
      }));

      setForecastData(forecastData);
    }
  }, [hourlyData, hourlyTemp]); // Добавляем hourlyData и hourlyTemp в зависимости для эффекта
      ...
      {forecastData.map((item, index) => (
        <WeatherCard key={index}>
          <StyledBoxText>
            ...
            {item.icon} {/* Отображаем иконку погоды */}
          </StyledBoxText>
        </WeatherCard>
      ))}
    </Container>
