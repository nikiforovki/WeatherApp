...
const HourlyWeatherForecast = () => {
  ...
  const [loading, setLoading] = useState(false); // Добавлен новый state для отслеживания загрузки
  ...
  useEffect(() => {
    setLoading(true); // Установка состояния загрузки в true перед началом запроса
    if (hourlyTemp && hourlyTemp.length >= 7) {
      ...
      setIsDataLoaded(true); // Установка состояния загрузки в false после успешной загрузки данных
      setLoading(false);
    }
  ...
  return (
    <Container>
      <TemperatureScaleToggle
        onScaleChange={() =>
          dispatch(/* Здесь должно быть действие toggleTemperatureScale */)
        }
      />
      {forecastData.map((item, index) => (
        <WeatherCard key={index}>
          {loading || !isDataLoaded ? ( // Изменено условие для отображения скелетона
            <SkeletonTheme color='grey'>
              <SkeletonWrapper>
                <StyledSkeleton height='100%' width='100%' />
              </SkeletonWrapper>
            </SkeletonTheme>
          ) : (
            <>
