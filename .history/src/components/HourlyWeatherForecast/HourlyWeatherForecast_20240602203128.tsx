...
const HourlyWeatherForecast = () => {
  ...
  useEffect(() => {
    if (hourlyTemp && hourlyTemp.length >= 7) {
      ...
      // Создаем новые объекты Date для каждого временного интервала и сдвигаем их на 7 часов назад
      const times = temps.map((_, index) => {
        const now = new Date(); // Текущее время
        now.setHours(now.getHours() + 7); // Сдвигаем на 7 часов назад
        return formatDate(now); // Форматируем дату и время
      });

      setForecastData(
        temps.map((temp, index) => ({
          time: times[index], // Используем сдвинутое время
          temperature: Math.round(temp),
          iconCode: icons[index],
        })),
      );
