...
const HourlyWeatherForecast = () => {
  ...
  useEffect(() => {
    const updatedForecastData = forecastData.map((item, index) => {
      ...
      if (index === 0) {
        return { ...item, temperature: firstSevenTemps[0] };
      }
  ...
  }, [hourlyTemp]); // Зависимость от hourlyTemp
        ...
        <WeatherCard key={index}>
          <StyledBoxText>
            ...
            <StyledIconСloudСomputing>
              {hourlyData && getWeatherIcon(hourlyData.description)}{' '}
              {/* Убедитесь, что hourlyData содержит поле description */}
            </StyledIconСloudСomputing>
          </StyledBoxText>
        </WeatherCard>
      ))}
