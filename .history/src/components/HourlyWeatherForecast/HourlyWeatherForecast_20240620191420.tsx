...
const HourlyWeatherForecast = () => {
  ...
  // console.log('Horly', descriptionIconsHuorly);
  ...
  useEffect(() => {
    ...
    if (hourlyTemp && hourlyTemp.length >= 7) {
      // Убираем условие для преобразования температуры
      const temps = hourlyTemp.slice(0, 7);
      const icons = descriptionIconsHuorly.slice(0, 7);
      setForecastData(
        temps.map((temp, index) => ({
          time: ` ${index + 1}`,
          temperature: Math.round(temp), // Теперь temp уже является округленным значением
