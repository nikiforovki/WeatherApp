...
const HourlyWeatherForecast = () => {
  ...
  const forecastData = useSelector(selectHourlyData);
  const descriptionIconsHuorly = useSelector(selectDescriptionIconHurly);
  const error = useSelector((state: RootState) => state.error);
  const [showError, setShowError] = useState(false);
  const [isDataLoaded, setIsDataLoaded] = useState(false); // Начальное значение false
  ...
  useEffect(() => {
    if (hourlyTemp && hourlyTemp.length >= 7) {
      ...
      setIsDataLoaded(true); // Установка состояния загрузки в true после успешной загрузки данных
    } else {
      setIsDataLoaded(false); // Установка состояния загрузки в false, если данные еще не загружены
    }
