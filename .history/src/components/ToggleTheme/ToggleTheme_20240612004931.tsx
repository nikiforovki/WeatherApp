...
const ToggleTheme: React.FC = () => {
  ...
  useEffect(() => {
    // Функция для проверки и установки темы при монтировании и обновлении страницы
    ...
    // Добавляем слушатель на изменения в предпочтениях пользователя
    const prefersDarkScheme = window.matchMedia('(prefers-color-scheme: dark)');
    const updateThemeFromPreferences = (event) => {
      setIsDarkTheme(event.matches);
    };
    prefersDarkScheme.addEventListener('change', updateThemeFromPreferences);

    // Удаляем слушатель при размонтировании компонента
    return () => {
      prefersDarkScheme.removeEventListener(
        'change',
