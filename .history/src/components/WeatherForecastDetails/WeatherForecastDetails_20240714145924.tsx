...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (error) {
      setIsLoading(true); // Устанавливаем isLoading в true при наличии ошибки
      setTimeout(() => {
        setIsLoading(false); // Сразу же устанавливаем isLoading в false, чтобы скрыть скелетон
        setShowError(true);
      }, 1000); // Задержка в 1000 мс перед отображением сообщения об ошибке
    } else {
      setIsLoading(true); // Устанавливаем isLoading в true при отсутствии ошибки
      setTimeout(() => {
        setIsLoading(false); // Скрытие скелетона через 1000 мс
        setShowError(false);
      }, 1000); // Задержка в 1000 мс перед установкой isLoading в false
    }
  }, [error]);
