...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (error) {
      ...
      // Здесь должна быть логика загрузки данных, например:
      // fetchWeatherData().then(() => setIsLoading(false));
      // После завершения загрузки данных устанавливаем isLoading в false
      setIsLoading(false);
    }
  }, [error]);
