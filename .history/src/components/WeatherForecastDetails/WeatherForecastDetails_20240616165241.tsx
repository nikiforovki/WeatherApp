    ...
    return (
      <SkeletonTheme color='#FFFFFF' highlightColor='#1AADE3'>
        <div style={{ padding: '20px' }}>
          {[...Array(8)].map((_, index) => (
            <Skeleton key={index} height={180} width={80} borderRadius={20} />
          ))}
        </div>
      </SkeletonTheme>
    );
    ...
    <Container>
      ...
      {cardsData.map((card, index) => (
        <WeatherCard key={index}>
          ...
          {index === sunriseIndex &&
            currentWeatherData.sunrise !== undefined && (
              <StyledValue>
                {new Date(
          ...
          {index === sunsetIndex && currentWeatherData.sunset !== undefined && (
            <StyledValue>
              {new Date(currentWeatherData.sunset * 1000).toLocaleTimeString()}
            </StyledValue>
          )}
          {index === humidityIndex &&
            currentWeatherData.humidity !== undefined && (
              <StyledValue>{currentWeatherData.humidity}%</StyledValue>
            )}
          {index === windIndex &&
            currentWeatherData.windSpeed !== undefined && (
              <StyledValue>
                {Math.round(currentWeatherData.windSpeed)} km/h
              </StyledValue>
            )}
          {index === pressureIndex &&
            currentWeatherData.pressure !== undefined && (
              <StyledValue>{currentWeatherData.pressure} hPa</StyledValue>
            )}
          {index === feelsLikeIndex &&
            currentWeatherData.feelsLike !== undefined && (
              <StyledValue>{currentWeatherData.feelsLike}°</StyledValue>
            )}
          {index === visibilityIndex &&
            currentWeatherData.visibility !== undefined && (
              <StyledValue>
                {Math.round(currentWeatherData.visibility / 1000)} km
              </StyledValue>
            )}
        </WeatherCard>
      ))}
