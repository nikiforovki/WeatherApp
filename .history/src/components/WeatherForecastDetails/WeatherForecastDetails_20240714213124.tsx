...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    console.log('Error:', error); // Логирование ошибки
    console.log('Is Loading:', isLoading); // Логирование состояния загрузки

    if (error) {
      setIsLoading(true);
      setTimeout(() => {
        setIsLoading(false); // Установка isLoading в false после таймаута
        setShowError(true);
      }, 300);
    } else {
      setIsLoading(false); // Установка isLoading в false, когда нет ошибки
      setShowError(false);
    }
  }, [error]);
  ...
  return (
    <div>
      ...
      <Container>
        {isLoading ? ( // Используйте isLoading вместо loading
          <SkeletonTheme color='#FFFFFF' highlightColor='#1AADE3'>
            <SkeletonWrapper>
              {[...Array(8)].map((_, i) => (
                <Skeleton key={i} height={140} width={280} borderRadius={20} />
              ))}
            </SkeletonWrapper>
          </SkeletonTheme>
        ) : (
