...
export const WeatherForecastDetails: React.FC = () => {
  ...
  console.log('Загрузка данных:', isLoading);
  ...
  useEffect(() => {
    if (error) {
      ...
      setIsLoading(true);
      // Вызываем действие для получения данных из хранилища
      dispatch(fetchDataFromStore()).then(() => {
        // После завершения асинхронной операции устанавливаем isLoading в false
        setIsLoading(false);
      });
    }
  }, [error, dispatch]); // Теперь dispatch включен в зависимости
