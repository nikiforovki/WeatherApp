...
export const WeatherForecastDetails: React.FC = () => {
  ...
  const sunrise = useSelector((state: RootState) => state.weather?.sunrise);
  const sunset = useSelector((state: RootState) => state.weather?.sunset);
  const humidity = useSelector((state: RootState) => state.weather?.humidity);
  const feelsLike = useSelector((state: RootState) => state.weather?.feelsLike);
  const roundFeelsLike = Math.round(feelsLike);
  const pressure = useSelector((state: RootState) => state.weather?.pressure);
  const precipitation = useSelector(
    (state: RootState) => state.weather?.precipitation,
  );
  const visibility = useSelector(
    (state: RootState) => state.weather?.visibility,
  );
  ...
  console.log('Загрузка компанента:', isLoading);
  ...
  if (isLoading) {
    return (
      <SkeletonTheme color='#888' highlightColor='#black'>
        <div>
          {[...Array(8)].map((_, i) => (
