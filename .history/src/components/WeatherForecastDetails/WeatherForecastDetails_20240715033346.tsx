...
export const WeatherForecastDetails: React.FC = () => {
  ...
  //   if (error) {
  //     setIsLoading(true);
  //     console.log('Loading after error:', isLoading); // Логирование после установки isLoading в true
  //     setTimeout(() => {
  //       setIsLoading(false);
  //       setShowError(true);
  //       console.log('Loading after timeout:', isLoading); // Логирование после установки isLoading в false
  //       console.log('ShowError after timeout:', showError); // Логирование после установки showError в true
  //     }, 300);
  //   } else {
  //     setIsLoading(false);
  //     setShowError(false);
  //     console.log('Loading without error:', isLoading); // Логирование после установки isLoading в false без ошибки
  //     console.log('ShowError without error:', showError); // Логирование после установки showError в false без ошибки
  //   }
  // }, [error, isLoading]); // Обратите внимание, что isLoading здесь не нужен, так как он не влияет на эффект

  useEffect(() => {
    let timer;
    if (error) {
      setIsLoading(true);
      console.log('Loading after error:', isLoading);
      timer = setTimeout(() => {
        setIsLoading(false);
        setShowError(true);
        console.log('ShowError after timeout:', showError);
      }, 300);
    } else {
      setIsLoading(false);
      console.log('Loading without error:', isLoading);
      setShowError(false);
      console.log('ShowError without error:', showError);
    }
