...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (error) {
      ...
      setIsLoading(false); // Сброс состояния загрузки вне зависимости от наличия ошибки
    }
  }, [error]);

  if (showError) {
    return <div>Error: {error.message}</div>; // Оставляем один вывод сообщения об ошибке
  }
