...
export const WeatherForecastDetails: React.FC = () => {
  ...
  const visibilityKm = weatherData?.visibility / 1000 || 'Неизвестно';

  const sunsetTime = weatherData?.sunset
    ? new Date(weatherData.sunset * 1000)
  ...
  const sunriseTime = weatherData?.sunrise
    ? new Date(weatherData.sunrise * 1000)
        .toLocaleTimeString('en-US', {
          hour: '2-digit',
          minute: '2-digit',
        })
    ...
    <Container>
      {cardsData.map((card, index) => (
        <WeatherCard key={index}>
          ...
          {index === 0 && weatherData?.sunrise !== undefined && (
            <StyledValue>{sunriseTime}</StyledValue>
          )}

          {index === 1 && weatherData?.sunset !== undefined && (
            <StyledValue>{sunsetTime}</StyledValue>
          )}

          {index === 2 && weatherData?.precipitation !== undefined && (
            <StyledValue>{weatherData.precipitation} % </StyledValue>
          )}

          {index === 3 && weatherData?.humidity !== undefined && (
            <StyledValue>{weatherData.humidity}%</StyledValue>
          )}

          {index === 4 && weatherData?.windSpeed !== undefined && (
            <StyledValue>{Math.round(weatherData.windSpeed)} km/h</StyledValue>
          )}

          {index === 5 && weatherData?.pressure !== undefined && (
            <StyledValue>{weatherData.pressure} hPa</StyledValue>
          )}

          {index === 6 && weatherData?.feelsLike !== undefined && (
            <StyledValue>{Math.round(weatherData.feelsLike)}°</StyledValue>
          )}

          {index === 7 && weatherData?.visibility !== undefined && (
