...
export const WeatherForecastDetails: React.FC = () => {
  ...
  console.log('Селектор', pressure);
  const visibility = useSelector((state) => state.weatherData.main.visibility);
    ...
    <Container>
      {cardsData.map((card, index) => (
        <WeatherCard key={index}>
          ...
          {index === humidityIndex && humidity !== null && (
            <StyledValue>{humidity}%</StyledValue>
          )}
          {index === windIndex && windSpeed !== undefined && (
            <StyledValue>{windSpeed} m/s</StyledValue>
          )}
        </WeatherCard>
      ))}
    </Container>
