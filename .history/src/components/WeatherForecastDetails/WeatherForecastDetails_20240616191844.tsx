import React, { useEffect, useState } from 'react'; // Исправлено: добавлены пробелы вокруг равенства
...
export const WeatherForecastDetails: React.FC = () => {
  const [isLoading, setIsLoading] = useState(false); // Исправлено: добавлены пробелы вокруг равенства
    ...
    <Container>
      ...
      {cardsData.map((card, index) => (
        <WeatherCard key={index}>
          ...
          {Object.keys(indices).includes(card.description) &&
            indices[card.description] !== undefined && (
              <StyledValue>{indices[card.description]} </StyledValue>
            )}
        </WeatherCard>
      ))}
    </Container>
