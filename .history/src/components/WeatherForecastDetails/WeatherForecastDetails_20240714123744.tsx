...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (error) {
      setShowError(true); // Установите showError в true, когда ошибка присутствует
    } else {
      setShowError(false); // Сбросьте состояние, когда ошибка исчезла
    }
  }, [error]); // Зависимость от ошибки

  if (showError) {
    return <div>Город не найден</div>; // Отображайте сообщение об ошибке, когда showError истинно
  }
