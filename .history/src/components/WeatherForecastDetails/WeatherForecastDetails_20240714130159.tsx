...
export const WeatherForecastDetails: React.FC = () => {
  ...
  console.log('Показывает что есть ошибка:', error);
  ...
  useEffect(() => {
    if (error) {
      setIsLoading(true); // Устанавливаем isLoading в true, когда есть ошибка
      setTimeout(() => {
        setIsLoading(false); // Сбрасываем isLoading после задержки
        setShowError(true); // Устанавливаем showError в true, когда есть ошибка
      }, 300);
    } else {
      setIsLoading(false); // Сбрасываем isLoading, когда ошибка исчезает
      setShowError(false); // Сбрасываем showError, когда ошибка исчезает
    }
  }, [error]);

  if (showError) {
    return <div>Город не найден</div>; // Отображаем сообщение об ошибке, когда showError истинно
  }
...
//   useEffect(() => {
//     console.log('Загрузка скелетона', isLoading);
//   }, [isLoading]);
...
//   }, [error]);
//   console.log('Should render skeleton:', isLoading);
//   if (isLoading) {
//     return (
//       <SkeletonTheme color='#888' highlightColor='#ddd'>
//         <SkeletonWrapper>
