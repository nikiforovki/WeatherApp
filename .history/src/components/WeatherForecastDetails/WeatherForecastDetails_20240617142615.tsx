...
export const WeatherForecastDetails: React.FC = () => {
  ...
  const [dataLoaded, setDataLoaded] = useState(false); // Добавлена новая переменная состояния
  ...
  useEffect(() => {
    if (error) {
      ...
      setDataLoaded(false); // Установка dataLoaded в true после загрузки данных
    }
  }, [error, weatherData]);

  useEffect(() => {
    if (dataLoaded) {
      setIsLoading(false); // Установка isLoading в false только после установки dataLoaded в true
    }
  }, [dataLoaded]);
