...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (error) {
      setIsLoading(true);
      setTimeout(() => {
        setIsLoading(false); // Установка в false после задержки, чтобы прекратить отображение скелетона
        setShowError(true);
      }, 300);
    } else {
      setIsLoading(false); // Установка в false, чтобы перейти к основному содержимому
      setShowError(false);
    }
  }, [error]);
