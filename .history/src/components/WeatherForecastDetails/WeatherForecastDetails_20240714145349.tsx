...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (error) {
      ...
      setIsLoading(true); // Установка в true перед началом загрузки данных
      fetchData()
        .then(() => {
          setTimeout(() => {
            // Задержка перед установкой в false
            setIsLoading(false);
            setShowError(false);
          }, 500); // Задержка в 500 мс, например
        })
        .catch((error) => {
          setIsLoading(false); // Установка в false после ошибки
          setShowError(true);
        });
    }
  }, [error]);
