...
export const WeatherForecastDetails: React.FC = () => {
  ...
  console.log('Данные', weatherData);
  ...
  useEffect(() => {
    if (error) {
      setShowError(true);
      // Здесь можно установить isLoading в true, если это необходимо для вашего случая
    } else {
      // Сбросить isLoading в false, когда нет ошибки и данные загружены
      setIsLoading(false);
    }
  }, [error]);

  // Удалите второй useEffect, так как он дублирует логику первого и может вызвать путаницу
