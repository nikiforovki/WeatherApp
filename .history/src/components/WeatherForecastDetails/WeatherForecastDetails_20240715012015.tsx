...
export const WeatherForecastDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (error) {
      setTimeout(() => {
        setIsLoading(true); // Это должно устанавливать состояние в true, когда происходит ошибка
      }, 10000);
    } else {
      setIsLoading(false); // Убедитесь, что здесь состояние корректно сбрасывается
    }
  }, [error]);
