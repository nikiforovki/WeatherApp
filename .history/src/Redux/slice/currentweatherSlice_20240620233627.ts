...
export const selectTemperature = (state) => {
  const temperatures = state.weatherData.hourly; // Или другой путь до массива температур в вашем состоянии
  return temperatures && temperatures.length > 0 ? temperatures[0].temp : null;
};
