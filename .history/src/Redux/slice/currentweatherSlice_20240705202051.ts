...
const initialState = {
  ...
  error: null,
  city: '',
  weatherUnit: 'metric', // Используйте 'C' для начала работы с градусами Цельсия
...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(toggleTemperatureScale, (state) => {
      const newScale = state.weatherUnit === 'metric' ? 'imperial' : 'metric'; // Используйте weatherUnit для определения текущего масштаба
      state.weatherUnit = newScale; // Обновите weatherUnit, а не temperatureScale
...
export const selectTemperature = createSelector(
  ...
  (weatherData, temperatureScale) => {
    ...
    return temperatureScale === 'C' ? temp : temp;
    // return temperatureScale === 'C' ? temp : (temp * 9) / 5 + 32;
  },
);
// export const selectWeatherUnit = (state) => state.weatherData.weatherUnit;
export const selectWeatherUnit = (state) => {
  console.log('State in selectWeatherUnit:', state);
  return state.weather && state.weather.weatherUnit;
};
