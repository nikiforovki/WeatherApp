...
export const selectHourlyTemp = (state) => {
  console.log('selectHourlyTemp called'); // Добавьте логирование для отладки

  if (!state.weatherData || !state.weatherData.hourly) {
    console.log('No weather data available'); // Логирование для отладки
    return [];
  }

  // Извлекаем температуру из каждого элемента без преобразования
  const temps = state.weatherData.hourly.map((item) => item.temp);
  console.log(temps); // Логирование результата для отладки
  return temps;
};
