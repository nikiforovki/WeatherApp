...
const initialState = {
  ...
  error: null,
  city: '',
  temperatureScale: 'C', // Изначально установлено на 'C' для метрической системы
  scale: 'metric', // Добавлен новый параметр для отслеживания системы измерения
  currentTemperatureCelsius: 0,
  currentTemperatureFahrenheit: 0,
  cities: [],
};
...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(putWeatherData, (state, action) => {
      const { currentTemp, windSpeed, unit } = action.payload;
      state.weatherData = action.payload;
      state.currentTemperatureCelsius = unit === 'metric' ? currentTemp : null;
      state.currentTemperatureFahrenheit =
        unit === 'imperial' ? currentTemp : null;
      state.windSpeedMph = unit === 'imperial' ? windSpeed : null;
      state.windSpeedMps = unit === 'metric' ? windSpeed : null;
    })
    ...
    .addCase(toggleTemperatureScale.type, (state) => {
      const newScale = state.scale === 'metric' ? 'imperial' : 'metric';
      console.log('Новая шкала:', newScale);
      state.scale = newScale; // Обновляем систему измерения

      if (newScale === 'imperial') {
        state.currentTemperatureCelsius =
          ((state.currentTemperatureFahrenheit - 32) * 5) / 9;
        state.currentTemperatureFahrenheit = state.currentTemperatureFahrenheit;
      } else {
        state.currentTemperatureCelsius = state.currentTemperatureCelsius;
        state.currentTemperatureFahrenheit =
...
export const selectTemperatureScale = (state) => {
  return state && state.weather ? state.weather.temperatureScale : 'C'; // Значение по умолчанию
};
