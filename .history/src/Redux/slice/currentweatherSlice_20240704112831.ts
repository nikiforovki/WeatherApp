...
import {
  ...
  updateCity,
  putWeatherData,
  receiveMetricWeatherData, receiveImperialWeatherData,
...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    builder.addCase(receiveMetricWeatherData, (state, action: PayloadAction<any>) => {
      state.metricData = action.payload;
    });
    builder.addCase(receiveImperialWeatherData, (state, action: PayloadAction<any>) => {
      state.imperialData = action.payload;
    });
