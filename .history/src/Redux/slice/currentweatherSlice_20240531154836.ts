...
export const selectTemperature = createSelector(
  ...
  (weatherData) => {
    ...
    const temperatureScale = state.weatherData.temperatureScale; // Исправлено здесь
    return temperatureScale === 'C' ? temp : (temp * 9) / 5 + 32; // Конвертация в Фаренгейты, если необходимо
  },
);
