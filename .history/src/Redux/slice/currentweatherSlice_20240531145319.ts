...
const initialState = {
  ...
  error: null,
  city: '',
  temperatureScale: 'C', // Начальное состояние масштаба температуры
...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(fetchCurrentWeatherSuccess, (state, action) => {
      console.log('Received weather data:', action.payload); // Логирование полученных данных
      state.weatherData = action.payload;
      state.error = null;
    })
    .addCase(fetchCurrentWeatherFailure, (state, action) => {
    ...
    .addCase(putWeatherData, (state, action) => {
      console.log('Action payload for putWeatherData:', action.payload); // Логирование action.payload
      const { currentTemp, windSpeed, weatherUnit } = action.payload;
      state.weatherData = action.payload;
      console.log('Action payload for putWeatherData:', action.payload); // ��огирование
    ...
    .addCase(toggleTemperatureScale, (state, action) => {
      state.temperatureScale = action.payload === 'C' ? 'F' : 'C';
    });
});
