...
const weatherReducer = createReducer(initialState, (builder) => {
  ...
  .addCase(fetchCurrentWeatherRequest, (state) => {
    state.error = null;
    state.isLoading = true; // Установка в true перед началом загрузки
  })
  .addCase(fetchCurrentWeatherSuccess, (state, action) => {
    state.weatherData = action.payload;
    state.error = null;
    state.isLoading = false; // Установка в false после успешной загрузки
  })
  .addCase(fetchCurrentWeatherFailure, (state, action) => {
    state.error = action.payload.message;
    state.isLoading = false; // Установка в false после ошибки
  })
  // Добавьте аналогичные изменения для других действий, если необходимо
...
export const setLoading = createAction<boolean>('weather/setLoading');
