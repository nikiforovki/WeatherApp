...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(toggleTemperatureScale, (state) => {
      // Переключаем масштаб температуры
      state.temperatureScale = state.temperatureScale === 'C' ? 'F' : 'C';

      // Конвертируем текущую температуру в соответствии с новым масштабом
      if (state.temperatureScale === 'F') {
        // Если новый масштаб - Фаренгейты, конвертируем из Цельсия
        state.currentTemperatureFahrenheit =
          ((state.currentTemperatureCelsius - 32) * 5) / 9;
      } else {
        // Если новый масштаб - Цельсии, конвертируем из Фаренгейтов
        state.currentTemperatureCelsius =
