...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(toggleTemperatureScale, (state, action) => {
      const newScale = action.payload; // Предполагается, что action.payload содержит новое значение масштаба ('C' или 'F')
      state.temperatureScale = newScale;
      if (newScale === 'F') {
        state.currentTemperatureCelsius =
          (((state.currentTemperatureCelsius || 0) - 32) * 5) / 9;
      } else {
        state.currentTemperatureFahrenheit =
          ((state.currentTemperatureCelsius || 0) * 9) / 5 + 32;
      }
    });
});
