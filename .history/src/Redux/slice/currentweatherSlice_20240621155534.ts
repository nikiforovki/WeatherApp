...
// Селектор для получения данных о погоде
const selectWeatherData = (state) => state.weatherData;

// Селектор для получения текущего масштаба температуры
const selectTemperatureScale = (state) => state.temperatureScale;

// Объединяем селекторы в один, используя createSelector
export const selectTemperature = createSelector(
  [selectWeatherData, selectTemperatureScale],
  (weatherData, temperatureScale) => {
    // Проверяем, есть ли данные о погоде и текущая погода
    if (!weatherData || !weatherData.current) {
      return null;
    }

    // Получаем температуру из данных о погоде
    const temp = weatherData.current.temp;

    // Преобразуем температуру в зависимости от выбранного масштаба
