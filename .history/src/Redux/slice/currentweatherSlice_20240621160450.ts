...
export const selectTemperature = createSelector(
  [selectWeatherData, selectTemperatureScale],
  (weatherData, temperatureScale, memoizedPreviousState) => {
    // Используем shallowEqual для сравнения предыдущего и текущего состояния
    if (
      shallowEqual(weatherData, memoizedPreviousState[0]) &&
      temperatureScale === memoizedPreviousState[1]
    ) {
      return memoizedPreviousState[2];
    }
  ...
  (weatherData, temperatureScale) => [weatherData, temperatureScale, null], // Инициализируем состояние для мемоизации
);
