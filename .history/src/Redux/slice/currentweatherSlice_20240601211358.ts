...
// export const selectErrorMessage = (state) => state.yourFeatureKey.error;

// В вашем файле с селекторами или в том же файле, где определен ваш редьюсер
export const selectErrorMessage = (state: string) => {
  // Проверяем, существует ли поле error в состоянии
  if (
    !state ||
    !state.currentWeather ||
    typeof state.currentWeather.error !== 'string'
  ) {
    return null; // Возвращаем null или другое значение по умолчанию, если поле отсутствует
  }
  return state.currentWeather.error;
};
