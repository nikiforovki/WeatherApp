...
// Селектор для температуры в Цельсиях
export const selectTemperatureCelsius = createSelector(
...
// Селектор для температуры в Фаренгейтах
export const selectTemperatureFahrenheit = createSelector(
  (state) => state.weather.weatherData,
  (weatherData) => {
    if (!weatherData || !weatherData.current) {
      return null;
    }
    const tempCelsius = weatherData.current.temp;
    return (tempCelsius * 9) / 5 + 32;
  },
);

// Селектор для выбора текущей температуры в зависимости от шкалы
export const selectTemperature = createSelector(
  (state) => state.weather.weatherData,
  (state) => state.weather.temperatureScale,
  (weatherData, temperatureScale) => {
    ...
    const tempCelsius = weatherData.current.temp;
    return temperatureScale === 'C' ? tempCelsius : (tempCelsius * 9) / 5 + 32;
  },
);

// Другие селекторы, обновленные для правильного пути к состоянию
...
export const selectCity = createSelector(
  (state) => state.weather.city,
  (city) => city,
);

export const selectCities = createSelector(
  (state) => state.weather.cities,
  (cities) => cities,
);

export const selectCityNames = createSelector(
  (state) => state.weather.cities?.map((city) => city.name) ?? [],
  (names) => names,
);

export const selectCityCountryName = createSelector(
  (state) => state.weather.cities?.map((city) => city.country) ?? [],
  (names) => names,
);

export const selectFirstCityCountryName = createSelector(
  (state) => state.weather.cities?.[0]?.country_name,
