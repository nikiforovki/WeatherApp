...
import {
  ...
  updateCitiesList, // Убедитесь, что этот редьюсер импортирован
...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(fetchCityListSuccess, (state, action) => {
      state.citiesList = action.payload; // Обновите состояние с полученными городами
    })
    .addCase(fetchCityListFailure, (state, action) => {
      state.error = action.payload; // Обработайте ошибку
    })
    .addCase(toggleTemperatureScale, (state, action) => {
      state.temperatureScale = action.payload;
    })
    .addCase(updateCitiesList, (state, action) => {
      state.citiesList = action.payload; // Adjust according to your state structure
    }); // Убедитесь, что закрывающая скобка находится здесь
});
