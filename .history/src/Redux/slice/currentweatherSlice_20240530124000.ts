...
const weatherReducer = createReducer(initialState, (builder) => {
  builder
    ...
    .addCase(switchTemperatureUnit, (state, action) => { // Добавляем обработку действия переключения единиц измерения
      state.temperatureScale = action.payload; // Обновляем единицы измерения в состоянии
    });
});
