...
export const selectTemperature = createSelector(
  (state) => state.weatherData.hourly, // Получаем массив hourly
  (state) => state.temperatureScale, // Получаем масштаб температуры
  (hourlyTemps, temperatureScale) => {
    if (!hourlyTemps || hourlyTemps.length === 0) {
      return null;
    }

    // Берем первую температуру из массива
    const firstTemp = hourlyTemps[0].temp;
    return temperatureScale === 'C' ? firstTemp : (firstTemp * 9) / 5 + 32;
  },
);
