...
function* workerGetWeatherDataBySearch(action) {
  ...
  try {
    ...
    switch (weatherUnit) {
      case 'metric':
        currentTemperature = weatherResponse.data.current.temp - 273.15;
        console.log(`Текущая температура в Цельсиях: ${currentTemperature}°C`);
        break;
      case 'imperial':
        currentTemperature =
          ((weatherResponse.data.current.temp - 273.15) * 9) / 5 + 32;
        console.log(
          `Текущая температура в Фаренгейтах: ${currentTemperature}°F`,
        );
        break;
      default:
        currentTemperature = weatherResponse.data.current.temp;
        console.log(`Текущая температура в Кельвинах: ${currentTemperature}°K`);
    }
