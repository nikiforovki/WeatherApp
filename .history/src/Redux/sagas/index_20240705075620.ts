...
function* workerGetWeatherDataBySearch(action) {
  try {
    ...
    // Установка температуры в соответствии с выбранной единицей измерения
    if (weatherUnit === 'metric') {
      temperature = weatherResponse.data.current.temp; // Температура уже в градусах Цельсия
    } else if (weatherUnit === 'imperial') {
      temperature = weatherResponse.data.current.temp_f; // Преобразование в градусы Фаренгейта, если доступно
    }
    ...
    yield put(
      putWeatherData({
        ...
        units: weatherUnit, // Сохраняем выбранную единицу измерения
      }),
    );
  } catch (error) {
...
function* handleTemperatureScaleChange(action) {
  try {
    const newScale = action.payload;
    const weatherData = yield call(fetchWeatherData, newScale); // Replace with your API call
    yield put(updateWeatherData(weatherData));
  } catch (error) {
    console.error('Error updating weather data:', error);
  }
}

function* watchTemperatureScaleChange() {
  yield takeLatest('TOGGLE_TEMPERATURE_SCALE', handleTemperatureScaleChange);
}
...
export function* watchTemperatureScaleChange() {
  yield takeLatest(toggleTemperatureScale, handleTemperatureScaleChange);
}

export default function* rootSaga() {
  yield all([
    watchFetchCurrentWeather(),
    watchTemperatureScaleChange()
