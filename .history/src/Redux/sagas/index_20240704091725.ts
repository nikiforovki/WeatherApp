...
function* workerGetWeatherDataBySearch(action) {
  try {
    ...
    if (weatherUnit === 'metric') {
      // Ничего не делаем, так как температура уже в нужной системе
    } else if (weatherUnit === 'imperial') {
      // Просто выводим температуру, так как она уже в градусах Цельсия
      console.log(temperature); // Или другой метод вывода
    }
