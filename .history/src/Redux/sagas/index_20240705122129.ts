...
function* workerGetWeatherDataBySearch(action) {
  try {
    const { cityName } = action.payload; // Теперь action.payload содержит только cityName
    const weatherUnit = yield select((state) => state.weatherUnit); // Получаем выбранную шкалу из состояния
