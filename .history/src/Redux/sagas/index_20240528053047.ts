...
function* workerWeatherData(payload) {
  console.log('Сага запущена'); // Лог для проверки запуска саги
...
function* workerGetWeatherDataBySearch(payload) {
  console.log('Запуск саги с новым городом:', payload); // Логирование входных данных
  try {
    const { location, weatherUnit } = payload.location;
    const { data } = yield call(
      weatherApi.get,
    ...
    console.log('Ответ от API:', data); // Логирование ответа от API
    const lat = data.coord.lat;
    const lon = data.coord.lon;
    const weatherDataCall = yield call(
      weatherApi.get,
    ...
    console.log('Данные о погоде:', weatherDataCall.data); // Логирование данных о погоде
    yield put(putWeatherData(weatherDataCall.data));
  } catch (error) {
