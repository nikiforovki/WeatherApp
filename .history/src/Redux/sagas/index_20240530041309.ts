...
function* fetchCurrentWeather(action) {
  ...
  try {
    ...
    // Проверяем, есть ли в ответе данные о погоде
    ...
    // Обрабатываем ошибку, генерируем специфическое сообщение об ошибке
    ...
    yield put(fetchCurrentWeatherFailure(errorMessage));
  }
}
...
function* workerGetWeatherDataBySearch(action) {
  ...
  try {
    ...
    // Убедитесь, что вы используете error.message для доступа к вашему собственному сообщению об ошибке
    const errorMessage = error.message;
    console.log('Подробное сообщение об ошибке:', errorMessage);

    // Проверяем статус ошибки и выводим сообщение о том, что город не найден
