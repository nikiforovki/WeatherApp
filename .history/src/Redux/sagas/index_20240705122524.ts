...
function* workerGetWeatherDataBySearch(action) {
  try {
    const { cityName } = action.payload; // Извлекаем cityName из action.payload
    const weatherUnit = yield select((state) => state.weatherUnit); // Используем select для получения текущей выбранной шкалы из состояния
