...
function* workerGetWeatherDataBySearch(action) {
  ...
  try {
    ...
    // Предполагается, что fetchWeatherDataSuccess - это действие, предназначенное для обновления состояния погоды
    yield put(fetchWeatherDataSuccess(weatherResponse.data));
  } catch (error) {
