...
function* workerGetWeatherDataBySearch(action) {
  try {
    ...
    // Сначала получаем координаты для названия города
    ...
    // Затем получаем данные о погоде используя эти координаты
    const { data: weatherData } = yield call(
      weatherApi.get,
      `onecall?lat=${lat}&lon=${lon}&units=${weatherUnit}&appid=${API_KEY}`,
    );

    // Отправляем действие для обновления данных о погоде в Redux store
