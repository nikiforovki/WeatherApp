...
function* fetchCityList(action) {
  try {
    console.log('Запуск fetchCityList саги'); // Лог для начала выполнения саги
    const response = yield call(
      axios.get,
      `http://api.openweathermap.org/geo/1.0/direct?q=London&limit=10&appid=${API_KEY}`,
    );
    console.log('Ответ от API:', response.data); // Лог для вывода ответа от API
    const cities = response.data;
    yield put(fetchCityListSuccess(cities));
    console.log('fetchCityListSuccess вызван с данными:', cities); // Лог после успешного вызова fetchCityListSuccess
  } catch (error) {
    console.error('Ошибка в fetchCityList:', error); // Лог ошибки
    yield put(fetchCityListFailure(error.message));
    console.log(
      'fetchCityListFailure вызван с сообщением об ошибке:',
      error.message,
    ); // Лог после вызова fetchCityListFailure
  }
}
