...
function* workerGetWeatherDataBySearch(action) {
  try {
    const { cityName, weatherUnits } = action.payload; // Изменено на массив weatherUnits
    let temperatures = {};
    ...
    const weatherResponses = []; // Массив для хранения ответов от API

    for (const unit of weatherUnits) {
      const weatherResponse = yield call(
        weatherApi.get,
        `onecall?lat=${lat}&lon=${lon}&units=${unit}&appid=${API_KEY}`,
      );
      temperatures[unit] = weatherResponse.data.current.temp;
      weatherResponses.push(weatherResponse);
    }

    yield put(
      putWeatherData({
        ...weatherResponses[0].data, // Используется первый ответ для общих данных
        current: weatherResponses.map((response) => ({
          ...response.data.current,
          temp: temperatures[response.data.units], // Обновляем температуру в каждом ответе
        })),
      }),
    );
  } catch (error) {
