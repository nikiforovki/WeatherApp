...
function* workerGetWeatherDataBySearch(action) {
  ...
  try {
    ...
    if (error.response && error.response.status === 404) {
      ...
      // Создаем новый объект с обновленным сообщением об ошибке
      const newErrorMessage = { message: 'Город не найден' };
      yield put(fetchWeatherDataByCityNameFailure(newErrorMessage));
    }
