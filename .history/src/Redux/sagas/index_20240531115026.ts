...
function* workerGetWeatherDataBySearch(action) {
  ...
  try {
    ...
    // Извлечение текущей температуры в стандартной шкале (Кельвины)
    const currentTemperatureKelvin = weatherResponse.data.current.temp;

    // Вычисляем текущую температуру в выбранной пользователем шкале
    let currentTemperature;
    switch (weatherUnit) {
      case 'metric':
        currentTemperature = weatherResponse.data.current.temp - 273.15; // Конвертация из Кельвин в Цельсий
        console.log(`Текущая температура в Цельсиях: ${currentTemperature}°C`); // Логирование температуры в Цельсиях
        break;
      case 'imperial':
        currentTemperature =
          ((weatherResponse.data.current.temp - 273.15) * 9) / 5 + 32; // Конвертация из Кельвин в Фаренгейт
        console.log(
          `Текущая температура в Фаренгейтах: ${currentTemperature}°F`,
        ); // Логирование температуры в Фаренгейтах
        break;
      default:
        currentTemperature = weatherResponse.data.current.temp; // Стандартная шкала (Кельвины)
        console.log(`Текущая температура в Кельвинах: ${currentTemperature}°K`); // Логирование температуры в Кельвинах
    }

    // Выводим текущую температуру в выбранной пользователем шкале
    console.log(
      `Текущая температура: ${currentTemperature}°${weatherUnit === 'metric' ? 'C' : weatherUnit === 'imperial' ? 'F' : 'K'}`,
    );
