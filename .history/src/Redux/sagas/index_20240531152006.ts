...
function* workerGetWeatherDataBySearch(action) {
  ...
  try {
    ...
    const weatherResponse = yield call(
      weatherApi.get,
      `onecall?lat=${lat}&lon=${lon}&units=metric&appid=${API_KEY}`, // Установка units в metric для получения данных в градусах Цельсия
    );
    console.log('Данные о погоде:', weatherResponse.data);

    // Получаем текущую температуру в градусах Цельсия
    const currentTemperatureInCelsius = weatherResponse.data.current.temp;

    // Конвертируем температуру в кельвины, если это необходимо
    const currentTemperatureInKelvin =
      weatherResponse.data.current.temp + 273.15;

    console.log(
      `Текущая температура в градусах Цельсия: ${currentTemperatureInCelsius}`,
    );
    console.log(
      `Текущая температура в кельвинах: ${currentTemperatureInKelvin}`,
    );
    ...
    yield put(
      putWeatherData({
        ...
        current: {
          ...
          temp: currentTemperatureInCelsius,
        }, // Отправляем температуру в градусах Цельсия
        tempInKelvin: currentTemperatureInKelvin, // Добавляем дополнительное поле для температуры в кельвинах
      }),
    );
  } catch (error) {
