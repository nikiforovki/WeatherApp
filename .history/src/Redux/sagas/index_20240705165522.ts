...
function* workerGetWeatherDataBySearch(action) {
  try {
    ...
    // Установите units в 'imperial' для получения данных в градусах Фаренгейта
    const response = yield call(
      weatherApi.get,
      `weather?q=${cityName}&units=imperial&appid=${API_KEY}`,
    );
    ...
    // Теперь temperature уже в градусах Фаренгейта, поэтому нет необходимости в преобразовании

    yield put(
      putWeatherData({
        ...weatherResponse.data,
        current: { ...weatherResponse.data.current, temp: temperature }, // Используем сохранённую температуру
      }),
    );
  } catch (error) {
...
//       console.log('Используется метрическая шкала');
//     } else if (weatherUnit === 'imperial') {
//       console.log('Используется империйская шкала');
//       const temperatureInFahrenheit = ((temperature - 273.15) * 9) / 5 + 32;
//       console.log(
//         'Температура в градусах Фаренгейта:',
//         temperatureInFahrenheit,
//       );
//       temperature = temperatureInFahrenheit; // Сохраняем температуру в градусах Фаренгейта
//     }
