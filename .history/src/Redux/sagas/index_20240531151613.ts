...
function* workerGetWeatherDataBySearch(action) {
  ...
  try {
    ...
    console.log(`Текущая температура: ${currentTemperature}°${weatherUnit}`);

    yield put(
      putWeatherData({
        ...weatherResponse.data,
        current: { ...weatherResponse.data.current, temp: currentTemperature },
      }),
    );
  } catch (error) {
...
function convertTemperature(temp, unit) {
  switch (unit) {
    case 'metric':
      return temp - 273.15; // Цельсий
    case 'imperial':
      return ((temp - 273.15) * 9) / 5 + 32; // Фаренгейт
    default:
      return temp; // Кельвины
  }
}
// Пример использования функции для разных температур и единиц измерения
const kelvin = 300;
console.log(
  `Температура в ${kelvin}K в цельсиях:`,
  convertTemperature(kelvin, 'metric'),
);
console.log(
  `Температура в ${kelvin}K в фаренгейтах:`,
  convertTemperature(kelvin, 'imperial'),
);

const celsius = 20;
console.log(
  `Температура в ${celsius}°C в кельвинах:`,
  convertTemperature(celsius, 'metric'),
);
console.log(
  `Температура в ${celsius}°C в фаренгейтах:`,
  convertTemperature(celsius, 'imperial'),
);

const fahrenheit = 68;
console.log(
  `Температура в ${fahrenheit}°F в кельвинах:`,
  convertTemperature(fahrenheit, 'imperial'),
);
console.log(
  `Температура в ${fahrenheit}°F в цельсиях:`,
  convertTemperature(fahrenheit, 'metric'),
);
