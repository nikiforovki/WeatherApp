...
function* workerGetWeatherDataBySearch(action) {
  try {
    // Извлекаем cityName из action.payload
    const { cityName } = action.payload;

    // Используем select для получения текущей выбранной шкалы из состояния
    const weatherUnit = yield select((state) => state.weatherUnit);

    // Выполняем первый запрос к API для получения координат города
    const response = yield call(
      weatherApi.get,
      `weather?q=${cityName}&appid=${API_KEY}`,
    );

    // Извлекаем широту и долготу из ответа
    const { lat, lon } = response.data.coord;

    // Выполняем второй запрос к API с использованием выбранной шкалы
    const weatherResponse = yield call(
      weatherApi.get,
      `onecall?lat=${lat}&lon=${lon}&units=${weatherUnit}&appid=${API_KEY}`,
    );

    // Извлекаем текущую температуру из ответа
    const temperature = weatherResponse.data.current.temp;

    // Отправляем действие putWeatherData с обновленными данными о погоде
    yield put(
      putWeatherData({
    ...
    // Отправляем действие fetchWeatherDataByCityNameFailure с сообщением об ошибке
